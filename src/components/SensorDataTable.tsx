...
interface SensorDataTableProps {
  data: SensorData[];
  title?: string;
  refreshData?: () => void; // Optional callback to trigger data refresh
}
...
const SensorDataTable = ({ 
  ...
  // Keep track of the actual data count versus what's displayed in the UI
  ...
  // Update the data reference and counts when data prop changes
  useEffect(() => {
        ...
        <div className="rounded-md border overflow-auto">
          <Table>
            ...
            <TableBody>
              {paginatedData.length > 0 ? (
                paginatedData.map((item) => (
                  <TableRow key={item._id || item.timestamp}>
                    <TableCell className="font-medium">
                      {formatDateTime(item.timestamp as string)}
                    </TableCell>
                    <TableCell>{item.temperature.toFixed(1)}</TableCell>
